...
import Reconstruction.fastmri.data.transforms as fastmri_transforms
import numpy as np
import torch
...
def raw_transform_miccai2020(kspace=None, mask=None, **_kwargs):
    ...
    # NOTE: This is the k-space visualization code, if do not needed, it can pass to speed up, from 105 to 123
    masked_true_k_space_d = to_magnitude(masked_true_k_space, dim=1)
    masked_true_k_space_d = Reconstruction.fastmri.ifftshift(masked_true_k_space_d, dim=(1, 2))

    masked_true_k_space_d = masked_true_k_space_d.permute(0, 2, 1)
    masked_true_k_space_d = torch.flatten(masked_true_k_space_d, 0, 1)
    masked_true_k_space_d = masked_true_k_space_d.permute(1, 0)
    masked_true_k_space_d = np.array(masked_true_k_space_d)
    masked_true_k_space_d = np.log(masked_true_k_space_d + 1e-9) / \
                            np.max(np.max(np.log(masked_true_k_space_d + 1e-9)))
    masked_true_k_space_d[:, 368 - 1] = 255
    masked_true_k_space_d[:, 368 * 2 - 1] = 255
    masked_true_k_space_d[:, 368 * 3 - 1] = 255
    cv2.imshow("Trajectories", masked_true_k_space_d)
    cv2.waitKey(1)
...
def _base_fastmri_unet_transform(
    ...
    image = Reconstruction.fastmri.ifft2c(masked_kspace)
    ...
    # absolute value
    image = Reconstruction.fastmri.complex_abs(image)

    # apply Root-Sum-of-Squares if multicoil data
    if which_challenge == "multicoil":
        image = Reconstruction.fastmri.rss(image)
