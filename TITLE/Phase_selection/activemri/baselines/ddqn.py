...
import Phase_selection.activemri.envs.envs as mri_envs
import Phase_selection.activemri.experimental.cvpr19_models.models.evaluator as cvpr19_evaluator

from . import Policy, RandomPolicy, evaluation, replay_buffer

from Phase_selection.activemri.models.vit import MaskfusedViT
...
class SimpleMLP(nn.Module):
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        if self.ignore_mask:
            ...
            time_steps = previous_actions.sum(1).unsqueeze(1) - 66
...
class SSMaskfusedTransformer(nn.Module):
    """ Transformer Value network used for dataset specific DDQN model. """
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        Note:
            ...
            set to ``1e-10``.
        """
        reconstruction, mask, _ = _decode_obs_tensor(
            obs, 0
        )
        # print(mask.size())
        mask = mask.squeeze(1)
        mask = mask.squeeze(1)
        # reconstruction = reconstruction[..., 160:480, 24:344]
        # reconstruction = F.interpolate(reconstruction, [240, 240])
        # print("L340 ddqn recon dim", reconstruction.size())

        qvalue = self.evaluator(reconstruction, mask)
...
def _get_model(options):
    if options.dqn_model_type == "simple_mlp":
        return SimpleMLP(options.budget, options.image_width, ignore_mask=options.ignore_mask)
    ...
    if options.dqn_model_type == "ss_maskfusedtransformer":
        return SSMaskfusedTransformer(options.budget, options.image_width, options.image_height)
...
class DDQNTester:
    ...
    def __call__(self):
        ...
        while not training_done:
            ...
            test_scores, _, _ = evaluation.evaluate(
                self.env,
                self.policy,
...
class DDQNTrainer:
    ...
    def __init__(
        ...
        self.options.image_height = self.env.kspace_height
        self.steps = 0
        self.episode = 0
        self.best_test_score = -np.inf
        self.device = device
