...
import Reconstruction.fastmri.data
import gym
import numpy as np
import torch
...
import Phase_selection.activemri.data.transforms
import Phase_selection.activemri.envs.masks
import Phase_selection.activemri.envs.util
...
class ActiveMRIEnv(gym.Env):
    ...
    def __init__(
        ...
        self._reconstructor: Phase_selection.activemri.models.Reconstructor = None
    ...
    def _init_from_config_dict(self, cfg: Mapping[str, Any]):
        ...
        if not os.path.isdir(self._data_location):
            default_cfg, defaults_fname = Phase_selection.activemri.envs.util.get_defaults_json()
        ...
        mask_func = Phase_selection.activemri.envs.util.import_object_from_str(cfg["mask"]["function"])
        self._mask_func = functools.partial(mask_func, cfg["mask"]["args"])

        # Instantiating reconstructor
        reconstructor_cfg = cfg["reconstructor"]
        reconstructor_cls = Phase_selection.activemri.envs.util.import_object_from_str(
        ...
        self._transform = Phase_selection.activemri.envs.util.import_object_from_str(
    ...
    def _compute_obs_and_score(
        ...
        reconstructor_input = reconstructor_input[0] # original reconstructor_input include recon and mask, discard mask
        reconstructor_input = reconstructor_input.permute(0, 2, 3, 1)

        with torch.no_grad():
            extra_outputs = dict()
            extra_outputs["reconstruction"] = reconstructor_input
            extra_outputs["uncertainty_map"] = []
            extra_outputs["mask_embedding"] = []
    ...
    def _process_tensors_for_score_fns(
        ...
        print("envs 407:", reconstruction.size(), ground_truth.size())
    ...
    def reset(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        # Display setting of trajectories
        cv2.namedWindow("Trajectories", cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
        w_rescale_factor = 1
        w_width = int(1472 / w_rescale_factor)
        w_height_t = int(640 / w_rescale_factor + 60)
        cv2.resizeWindow("Trajectories", w_width, w_height_t)
        cv2.moveWindow("Trajectories", 2560 - w_width, 1440 - w_height_t)

        # Display setting of the trajectories paired images
        cv2.namedWindow("Images", cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
        # w_width = int(1280 / w_rescale_factor)
        w_height = int(640 / w_rescale_factor + 100)
        cv2.resizeWindow("Images", w_width, w_height)
        cv2.moveWindow("Images", 2560 - w_width, 1500 - w_height - w_height_t)
    ...
    def step(
        ...
        self._current_mask = Phase_selection.activemri.envs.masks.update_masks_from_indices(
        ...
        # # NOTE: This is the k-space visualization code, if do not needed, it can pass to speed up, from 638 to 653
        # Paired images visualization
        images_to_display = obs["reconstruction"].cpu()
        images_to_display = Phase_selection.activemri.data.transforms.to_magnitude(images_to_display, dim=3)
        images_to_display = images_to_display.permute(0, 2, 1)
        images_to_display = torch.flatten(images_to_display, 0, 1)
        images_to_display = images_to_display.permute(1, 0)
        images_to_display = np.array(images_to_display)
        images_to_display[:, 368-1] = 255
        images_to_display[:, 368 * 2 - 1] = 255
        images_to_display[:, 368 * 3 - 1] = 255
        cv2.imshow("Images", images_to_display)
        cv2.waitKey(1)
        ...
        done = Phase_selection.activemri.envs.masks.check_masks_complete(self._current_mask)
    ...
    def try_action(
        ...
        if isinstance(action, int):
            action = [action for _ in range(self.num_parallel_episodes)]
        new_mask = Phase_selection.activemri.envs.masks.update_masks_from_indices(
            self._current_mask, action
        )
...
class MICCAI2020Env(ActiveMRIEnv):
    ...
    def _create_dataset(self) -> DataInitFnReturnType:
        ...
        test_path = root_path / "knee_singlecoil_train"
        ...
        if str("train") in str(test_path):
            print("warning, check if the test_path is correct!")
    ...
    def _process_tensors_for_score_fns(
        ...
        reconstruction = Phase_selection.activemri.data.transforms.to_magnitude(reconstruction, dim=3)
        ground_truth = Phase_selection.activemri.data.transforms.to_magnitude(ground_truth, dim=3)

        reconstruction = Phase_selection.activemri.data.transforms.center_crop(
            reconstruction, (self.CENTER_CROP_SIZE, self.CENTER_CROP_SIZE)
        )
        ground_truth = Phase_selection.activemri.data.transforms.center_crop(
    ...
    def render(self, mode="human"):
        ...
        rec = Phase_selection.activemri.data.transforms.center_crop(
...
class FastMRIEnv(ActiveMRIEnv):
    ...
    def _create_dataset(self) -> DataInitFnReturnType:
        root_path = pathlib.Path(self._data_location)
        datacache_dir = Phase_selection.activemri.envs.util.maybe_create_datacache_dir()
        ...
        train_data = Reconstruction.fastmri.data.SliceDataset(
            train_path,
            ActiveMRIEnv._void_transform,
        ...
        val_data = Reconstruction.fastmri.data.SliceDataset(
            val_path,
            ActiveMRIEnv._void_transform,
            challenge=self.challenge,
            num_cols=self.num_cols,
        ...
        test_data = Reconstruction.fastmri.data.SliceDataset(
            test_path,
            ActiveMRIEnv._void_transform,
            challenge=self.challenge,
            num_cols=self.num_cols,
