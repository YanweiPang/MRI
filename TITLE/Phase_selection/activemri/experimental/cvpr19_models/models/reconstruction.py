...
class ReconstructorNetwork(nn.Module):
    ...
    def data_consistency(self, x, input, mask):
        ...
        print("L279 reconstruction", x.shape, input.shape)
        fuse = (
            fft_utils.ifft(
    ...
    def forward(self, zero_filled_input, mask):
        ...
        print("L310 reconstruction.py", type(zero_filled_input), self.use_mask_embedding,)
        if self.use_mask_embedding:
        ...
        print("L322 reconstruction.py", encoder_input.shape)
        ...
        for cascade_block, (encoder, residual_bottleneck, decoder) in enumerate(
            ...
            print("L331 reconstruction", encoder_output.shape, encoder_input.shape)
            if cascade_block > 0:
            ...
            print("L337 reconstruction", residual_bottleneck_output.shape)


            decoder_output = decoder(residual_bottleneck_output)
            print("L341 reconstruction", decoder_output.shape)
        ...
        print("L356 reconstruction", type(reconstructed_image), type(uncertainty_map), type(mask_embedding))
