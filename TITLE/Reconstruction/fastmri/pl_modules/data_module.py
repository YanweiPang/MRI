...
import torch
from Reconstruction.fastmri.data import CombinedSliceDataset, SliceDataset
...
class FastMriDataModule(pl.LightningDataModule):
    ...
    def _create_data_loader(
        ...
        if is_train and self.combine_train_val:
            data_paths = [
                ...
                self.data_path / f"{self.challenge}_test",
            ]
            ...
            if data_partition in ("test", "challenge") and self.test_path is not None:
                ...
                data_path = self.data_path / f"knee_{self.challenge}_{data_partition}"
        ...
        if self.distributed_sampler:
            if is_train:
                ...
                sampler = Reconstruction.fastmri.data.VolumeSampler(dataset, shuffle=False)
    ...
    def prepare_data(self):
        ...
        if self.use_dataset_cache_file:
            if self.test_path is not None:
                ...
                test_path = self.data_path / f"knee_{self.challenge}_test"
            data_paths = [
                self.data_path / f"knee_{self.challenge}_train",
                # self.data_path / f"knee_{self.challenge}_val",
                self.data_path / f"knee_{self.challenge}_test",
                test_path,
            ]
            data_transforms = [
    ...
    def val_dataloader(self):
        return self._create_data_loader(
            ...
            self.val_transform, data_partition="test", sample_rate=1.0
        )
