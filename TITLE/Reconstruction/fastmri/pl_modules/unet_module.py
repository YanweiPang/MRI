...
import torch
from Reconstruction.fastmri.models import Unet
from torch.nn import functional as F

from .mri_module import MriModule

from Reconstruction.fastmri.losses import SSIMLoss
...
class UnetModule(MriModule):
    ...
    def training_step(self, batch, batch_idx):
        image, target, mean, std, _, _, max_value = batch
        output = self(image)
        output = output * std + mean
        target = target * std + mean
        # loss = F.l1_loss(output, target)
        loss = self.loss(output.unsqueeze(0), target.unsqueeze(0), max_value)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {
            ...
            "output": output,
            "target": target,
            "val_loss": self.loss(output.unsqueeze(0), target.unsqueeze(0), max_value),
        }
